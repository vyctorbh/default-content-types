/**
 *
 * @module Enums
 *
 * @preferred
 *
 * @description Module for enums types defined in SenseNet helps you to use enums with dot notation.
 *
 * This module is autogenerated from Sense/Net metadata (/Odata/$metadata)
 *
 * ```
 * let car = new ContentTypes.Car({
 *  Id: 1,
 *  Name: 'MyCar',
 *  DisplayName: 'My Car',
 *  Style: Style.Cabrio
 * });
 * ```
 */
export declare enum VersioningMode {
    Option0 = "0",
    Option1 = "1",
    Option2 = "2",
    Option3 = "3",
}
export declare enum InheritableVersioningMode {
    Option0 = "0",
    Option1 = "1",
    Option2 = "2",
    Option3 = "3",
}
export declare enum ApprovingMode {
    Option0 = "0",
    Option1 = "1",
    Option2 = "2",
}
export declare enum InheritableApprovingMode {
    Option0 = "0",
    Option1 = "1",
    Option2 = "2",
}
export declare enum SavingState {
    Finalized = "0",
    Creating = "1",
    Modifying = "2",
    ModifyingLocked = "3",
}
export declare enum GroupAttachments {
    email = "email",
    root = "root",
    subject = "subject",
    sender = "sender",
}
export declare enum EnableAutofilters {
    Default = "0",
    Enabled = "1",
    Disabled = "2",
}
export declare enum EnableLifespanFilter {
    Default = "0",
    Enabled = "1",
    Disabled = "2",
}
export declare enum Language {
    En = "en",
    Hu = "hu",
}
export declare enum MemoType {
    generic = "generic",
    iso = "iso",
    iaudit = "iaudit",
}
export declare enum Priority {
    Option0 = "1",
    Option1 = "2",
    Option2 = "3",
}
export declare enum Status {
    pending = "pending",
    active = "active",
    completed = "completed",
    deferred = "deferred",
    waiting = "waiting",
}
export declare enum QueryType {
    Public = "Public",
    Private = "Private",
}
export declare enum Gender {
    Option0 = "...",
    Female = "Female",
    Male = "Male",
}
export declare enum MaritalStatus {
    Option0 = "...",
    Single = "Single",
    Married = "Married",
}
